...
public class EmpDao {
	...
	public static int save(Emp e){
		...
		try{
			...
			PreparedStatement ps=con.prepareStatement("insert into Employee12(name,password,email,country) values (?,?,?,?)");
	...
	public static int update(Emp e){
		...
		try{
			Connection con=EmpDao.getConnection();
			PreparedStatement ps=con.prepareStatement("update Employee12 set name=?,password=?,email=?,country=? where id=?");
			ps.setString(1,e.getName());
			ps.setString(2,e.getPassword());
			ps.setString(3,e.getEmail());
			ps.setString(4,e.getCountry());
